...
function DiscoverContent() {
  ...
  // Themes: top collections with their recent items
  const themes = useMemo(() => {
    return collections.slice(0, 5).map(col => ({
      ...col,
      recentItems: items.filter(it => it.categories[0] === col.title).slice(0, 3),
    }));
  }, [collections, items]);
  ...
  return (
    <div style={{ height: '100%', overflow: 'auto', paddingBottom: 100 }}>
      ...
      {collections.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '0 18px', marginBottom: 10 }}>
            <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink)', margin: 0 }}>My collections</h3>
            <span style={{ fontSize: 11, color: 'var(--ink-3)' }}>{collections.length} folders</span>
          </div>
      ...
      {thisWeek.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '0 18px', marginBottom: 10 }}>
            <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink)', margin: 0 }}>This week</h3>
            <span style={{ fontSize: 11, color: 'var(--ink-3)' }}>{thisWeek.length} new saves</span>
          </div>
          <div style={{
            margin: '0 18px',
            ...
            {thisWeek.slice(0, 8).map((it, i) => (
              <div key={it.id} style={{
                ...
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-4)', fontVariantNumeric: 'tabular-nums', minWidth: 20 }}>
      ...
      {themes.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          <div style={{ padding: '0 18px', marginBottom: 10 }}>
            <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink)', margin: 0 }}>Themes in your library</h3>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, margin: '0 18px' }}>
            {themes.map(theme => (
              <div key={theme.title} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  <div style={{
                    ...
                    background: cardColor(theme.color),
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontFamily: 'var(--font-serif)', fontWeight: 700, fontSize: 14, color: 'var(--ink)',
                  }}>{theme.title[0].toUpperCase()}</div>
                  <div>
                    <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--ink)' }}>{theme.title}</div>
                    <div style={{ fontSize: 11, color: 'var(--ink-3)' }}>{theme.count} saves</div>
                  </div>
                </div>
                {theme.recentItems.length > 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {theme.recentItems.map(it => (
                      <div key={it.id} style={{ fontSize: 12, color: 'var(--ink-2)', display: 'flex', alignItems: 'center', gap: 6, overflow: 'hidden' }}>
                        <KindIcon kind={it.kind} size={11} />
